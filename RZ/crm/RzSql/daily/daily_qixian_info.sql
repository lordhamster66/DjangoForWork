SELECT a.qdate,a.term,sum(a.tz_r) tz_r,sum(a.tz_j) tz_j,sum(a.mb_ys) mb_ys
from
(
			SELECT DATE(a.time_h) qdate,a.term,count(DISTINCT a.uid) tz_r,sum(a.account) tz_j,0 mb_ys
			from
			(
				...
				else "F:6月标及以上" end term,a.time_h
				from
				(
		...
		GROUP BY DATE(a.time_h),a.term
		UNION ALL
		SELECT DATE(a.max_time) qdate,a.term,0 tz_r,0 tz_j,avg(a.time)
    from
		(
		...
		where h.max_time >= DATE_SUB(CURDATE(),INTERVAL 1 day)
		and h.max_time < CURDATE()
		) a
    GROUP BY DATE(a.max_time),a.term
) a
