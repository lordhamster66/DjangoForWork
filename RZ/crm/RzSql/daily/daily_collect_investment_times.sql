...
from
(
		SELECT h.uid,sum(h.recover_account) recover_account,b.cishu,
		case when b.cishu >= 2 and b.cishu <= 5 then "A:2-5次"
		when b.cishu >= 6 and b.cishu <= 10 then "B:6-10次"
		when b.cishu >= 11 and b.cishu <= 20 then "C:11-20次"
		when b.cishu >= 21 then "D:21次及以上" else "不存在" end lei
		from
		(
					# 没债转的在贷
