SELECT h.qixian term,count(DISTINCT(h.uid)) tz_r,sum(h.account) tz_j
from
(
SELECT hh.uid,hh.account,case when hh.qixian < 30 then "A:短标"
when hh.qixian = 30 then "B:1月标"
when hh.qixian > 30 and hh.qixian <= 60 then "C:2月标"
when hh.qixian > 60 and hh.qixian <= 90 then "D:3月标"
when hh.qixian > 90 and hh.qixian <= 180 then "E:6月标"
else "F:10月标" end qixian
from
(
SELECT a.uid,a.qixian,a.account
from
(
