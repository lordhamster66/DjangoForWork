SELECT a.uid,a.uname,a.mobile,a.time_h,s.update_time,q.name,t.account,t.time_h tz_time,t.qixian,t.name bd_name
from 01u_0info a
...
and unix_timestamp(t.time_h) >= unix_timestamp("{start_time}")
and unix_timestamp(t.time_h) < unix_timestamp("{end_time}")+86400
and q.name in ({name})
