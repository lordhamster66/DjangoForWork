...
q.name,a.account,a.time_h tz_time,a.qixian,a.name bd_name
from
(
...
and unix_timestamp(a.time_h) >= unix_timestamp("{start_time}")
and unix_timestamp(a.time_h) < unix_timestamp("{end_time}")+86400
and q.name in ({name})
