...
def register(request):
    ...
    if request.method == "GET":
        ...
        if register_obj.is_valid():
            ...
            pwd = m_obj.hexdigest()
            register_time = datetime.datetime.strftime(datetime.datetime.now(), "%Y-%m-%d %H:%M:%S")
            models.User.objects.create(username=username, pwd=pwd, qq=qq, register_time=register_time)
