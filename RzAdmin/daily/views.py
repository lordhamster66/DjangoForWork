...
from daily import utils
from automatic import models as automatic_models
...
class DataStorage(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        :return:
        """
        t = threading.Thread(target=self.daily_storage)
        t.start()  # 开始存储日报所需数据
        return HttpResponse("日报数据存储中!")
...
def rzjf_recorde(request):
    ...
    # 获取更新用SQL
    rzjf_first_invest_recorde_sql = automatic_models.SQLRecord.objects.filter(id=96).first().content
    rzjf_user_memo_sql = automatic_models.SQLRecord.objects.filter(id=98).first().content
    # 连接数据库并执行SQL
    cursor = connections['rz'].cursor()
    rzjf_first_invest_recorde_sql_list = utils.get_info_list("rz", rzjf_first_invest_recorde_sql)
    rzjf_user_memo_list = utils.get_info_list("rz", rzjf_user_memo_sql)
    with transaction.atomic():
        # 插入首投记录
...
def rzjf_invest_rank(request):
    """存储投资次数，用于获取第几次投资"""
    # 获取昨天的所有投资用户uid对应sql
    yesterday_invest_uid_sql = automatic_models.SQLRecord.objects.filter(id=99).first().content
    # 获取存储投资次数的sql
    rzjf_invest_rank_sql = automatic_models.SQLRecord.objects.filter(id=97).first().content
    # 获取昨天的所有投资用户uid
    yesterday_invest_uid = utils.get_info_list("rz", yesterday_invest_uid_sql)
