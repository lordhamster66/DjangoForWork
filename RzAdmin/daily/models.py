...
class BaseInfo(models.Model):
    """基础数据"""
    qdate = models.DateField(verbose_name='日期', unique=True, db_index=True)
...
class AssetInfo(models.Model):
    ...
    class Meta:
        unique_together = ("qdate", "term", "asset_type")
        verbose_name = '日报资产数据'
        verbose_name_plural = '日报资产数据'


class WithdrawClassify(models.Model):
    """日报所需提现分类数据详情"""
    qdate = models.DateField(verbose_name='日期', db_index=True)
    ...
    class Meta:
        unique_together = ("qdate", "term", "withdraw_type")
        verbose_name = '日报提现分类数据'
        verbose_name_plural = '日报提现分类数据'


class CollectClassify(models.Model):
    """日报所需待收分类数据详情"""
    qdate = models.DateField(verbose_name='日期', db_index=True)
    ...
    class Meta:
        unique_together = ("qdate", "term", "collect_type")
...
class TimeSlot(models.Model):
    ...
    class Meta:
        unique_together = ("qdate", "timeslot")
        verbose_name = "各时间段数据"
...
class UserRecoverAccount(models.Model):
    """每个用户的在贷详情"""
    qdate = models.DateField(verbose_name='日期', db_index=True)
    uid = models.IntegerField(verbose_name="用户ID", null=True)
    recover_account = models.DecimalField(max_digits=20, decimal_places=2, verbose_name="在贷金额", null=True)

    class Meta:
        unique_together = ("qdate", "uid")
...
class WDZJInfo(models.Model):
    ...
    class Meta:
        unique_together = ("qdate", "platName")
        verbose_name = '网贷之家数据信息'
        verbose_name_plural = '网贷之家数据信息'


class WDTYInfo(models.Model):
    """网贷天眼数据详情"""
    qdate = models.DateField(verbose_name='日期', db_index=True, null=True)
